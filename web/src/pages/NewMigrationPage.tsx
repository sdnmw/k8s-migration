...
export default function NewMigrationPage({ preview = false }: { preview?: boolean }) {
    ...
    if (current === 3) {
      ...
      try {
        const input: MigrationPlanInput = {
          ...
          sourceApplicationId: applicationId, assessmentId: assessment!.id, ...(mappingId ? { mappingProfileId: mappingId } : {}),
  ...
  const nextDisabled = current === 0 ? !sourceType || !sourceId : current === 1 ? !applicationId : current === 2 ? !targetId || (!!assessment && assessment.blockerCount > 0) : current === 3 ? !planName.trim() : current === 4 ? !preflight?.ready : !confirmed
...
function MappingStep({ planName, onPlanName, mappings, mappingId, onMapping, sourceType, volumeMode, onVolumeMode, preSync, onPreSync, httpChecks, onHTTPChecks }: { planName: string; onPlanName: (value: string) => void; mappings: MappingProfile[]; mappingId: string; onMapping: (value: string) => void; sourceType: 'KUBERNETES' | 'COMPOSE'; volumeMode: MigrationPlanInput['strategy']['volumeMode']; onVolumeMode: (value: MigrationPlanInput['strategy']['volumeMode']) => void; preSync: boolean; onPreSync: (value: boolean) => void; httpChecks: string; onHTTPChecks: (value: string) => void }) {
  ...
  return <><Title level={2}>目标映射与策略</Title><Paragraph>目标映射是可选的覆盖规则；未选择时自动创建目标 Namespace，并保留源资源值或使用目标集群默认值。</Paragraph><div className="wizard-form-grid"><Form.Item label="计划名称" required><Input value={planName} maxLength={128} onChange={(event) => onPlanName(event.target.value)} placeholder="例如：生产订单服务迁移" /></Form.Item><Form.Item label="目标映射（可选）"><Select allowClear value={mappingId || undefined} onChange={(value) => onMapping(value ?? '')} options={mappings.map((item) => ({ value: item.id, label: item.name }))} placeholder="不使用映射，自动配置目标" /></Form.Item><Form.Item label="卷迁移方式"><Select value={volumeMode} onChange={onVolumeMode} options={volumeOptions} /></Form.Item><Form.Item label="在线预同步"><Switch checked={preSync} onChange={onPreSync} checkedChildren="启用" unCheckedChildren="关闭" /></Form.Item></div><Form.Item label="HTTP 验证地址（每行一个，可选）"><Input.TextArea value={httpChecks} onChange={(event) => onHTTPChecks(event.target.value)} rows={3} placeholder="https://app.example.local/healthz" /></Form.Item><Alert showIcon type="info" title={sourceType === 'COMPOSE' ? 'Compose 会按项目名自动创建目标 Namespace；仅在需要覆盖默认值时选择映射。' : 'Kubernetes 仅对命中的规则执行改写；未命中的 Namespace 和资源值保持原样。'} /><Alert showIcon type="warning" title="最终同步会停止源业务；目标验证失败或人工切流前取消时，系统会恢复源端副本或 Compose 服务。" /></>
}
