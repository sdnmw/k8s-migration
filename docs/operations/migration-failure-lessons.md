...
- **Compose 项目漏发现**：发现同时读取 `docker compose ls --all --format json`，运行和已停止项目均纳入；发现失败返回实际权限或解析原因，不再报告“成功发现 0 个应用”。
- **`.env`、`env_file` 无权读取**：优先由远端 Docker Compose 在源主机执行 `config --format json`；普通用户失败时，仅对固定只读命令尝试现有 `sudo -n`。因此无需平台自动执行永久 `setfacl`，也不会静默修改源目录权限。
